...
class HTXClient:
    ...
    def _generate_signature(
        ...
        if not self.api_key or not self.api_secret:
            raise ValueError(
                "API key and secret are required for authenticated requests"
            )
    ...
    async def _make_request(
        ...
        try:
            ...
            if data.get("status") == "error":
                raise Exception(
                    f"HTX API error: {data.get('err-msg', 'Unknown error')}"
                )
    ...
    async def get_account_balance(self) -> Dict:
        ...
        return await self._make_request(
            "GET", "/v1/account/accounts/balance", signed=True
        )
    ...
    async def get_klines(
        self, symbol: str, period: str = "1day", size: int = 200
    ) -> Dict:
    ...
    async def place_order(
        ...
        return await self._make_request(
            "POST", "/v1/order/orders/place", params, signed=True
        )

    async def get_order_history(
        self, symbol: str = None, states: str = None, size: int = 100
    ) -> Dict:
        """Get order history"""
        params = {"size": size}
    ...
    async def get_trade_history(self, symbol: str = None, size: int = 100) -> Dict:
        ...
        return await self._make_request(
            "GET", "/v1/order/matchresults", params, signed=True
        )
    ...
    async def get_deposit_history(self, currency: str = None, size: int = 100) -> Dict:
        ...
        return await self._make_request(
            "GET", "/v1/query/deposit-withdraw", params, signed=True
        )
